...
adaptive_lr = tf.compat.v1.train.exponential_decay(args.lr, global_step, 100000, 0.96, staircase=False, name=None)

with tf.control_dependencies(update_ops):
    train_op = tf.compat.v1.train.AdamOptimizer(adaptive_lr).minimize(total_loss, global_step=global_step,
                                                            name='train_op')
train_summary = []
train_summary.append(tf.compat.v1.summary.scalar('train/total_loss', total_loss))
train_summary.append(tf.compat.v1.summary.scalar('train/Dice_pipeline', Dice_hard))
train_summary.append(tf.compat.v1.summary.image('Output Segmentation', y_out_dl))
train_summary.append(tf.compat.v1.summary.image('Ground Truth', y))
train_summary_op = tf.compat.v1.summary.merge(train_summary)
...
with tf.compat.v1.Session(config=config) as sess:
    if is_training:
        if restore:
            ...
            summary_writer_train = tf.compat.v1.summary.FileWriter(args.logdir + '/train', graph=sess.graph)
            summary_writer_valid = tf.compat.v1.summary.FileWriter(args.logdir + '/valid', graph=sess.graph)
        else:
        ...
        file = open(os.path.join(args.logdir, 'dice_train.txt'),'w')
        try:
            for iters in range(args.train_iter):
                ...
                if gb_step % args.train_sum_freq == 0:
                    ...
                    print('writing to file ' + str(Dicevalue))
                    file.write(str(Dicevalue) + '\n')
        ...
        for fullpath in glob.glob(os.path.join(directory, img_ext)):
            ...
            fig = plt.figure()
            #plt.suptitle('Dice is ' + str(f2))
            plt.imshow(seg_out_acm)
            #plt.show()
            plt.savefig(results_location + os.path.splitext(filename)[0]+".png")
